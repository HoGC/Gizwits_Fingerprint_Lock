...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flaglock)
    {
        info->event[info->num] = EVENT_lock;
        info->num++;
        dataPoints->valuelock = gizStandardDecompressionValue(lock_BYTEOFFSET,lock_BITOFFSET,lock_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuelock != cur->valuelock)
    {
        GIZWITS_LOG("valuelock Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(lock_BYTEOFFSET,lock_BITOFFSET,lock_LEN,(uint8_t *)devStatusPtr,dataPoints->valuelock);
